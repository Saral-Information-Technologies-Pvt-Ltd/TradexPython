...
class NewGttOrderRequest:
    ...
    def __post_init__(self):
        ...
        if self.main_trigger_price <= 0:
            raise ValueError("Main trigger price must be greater than zero.")
        
        if self.stop_trigger_price < 0:
            raise ValueError("Stop trigger price cannot be negative.")
...
@dataclass
class NewGttOrderResponse:
    status: int
    message: str
    data: Optional[NewGttOrderData]
